...
document.getElementById('Signup-form').addEventListener('submit', async (event) => {
  ...
  try {
    const response = await fetch(
      `http://172.104.207.190:8009/api/method/medkado.medkado.doctype.medkado_user.medkado_user.sign_up?email=${encodeURIComponent(email)}&password=${encodeURIComponent(password)}&mobile_no=${encodeURIComponent(mobileNumber)}&referral_code=${encodeURIComponent(referralCode)}&district=${encodeURIComponent(district)}`,
      {
        method: 'GET',
        headers: {
